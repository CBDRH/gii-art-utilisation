...
countries_remove <- c("SaudiArabia", "UnitedArabEmirates", "Jordan", 
                      "Lebanon", "Bahrain",
...
gni_blups <- predict(gni_imp, newdata = art_dat[miss_gni],allow.new.levels=TRUE)
...
sch_blups <- predict(sch_imp, art_dat[miss_sch],allow.new.levels=TRUE)
...
hdi_blups <- predict(hdi_imp, art_dat[miss_sch],allow.new.levels=TRUE)
...
vars <- c("country", "time", "ln_art_utilisation", 
          "hdi_un",
          "ln_gni_wb",
          "female_lfp_wb", "male_lfp_wb", "ratio_mfwork_wb", 
          "women_parl_wb",
...
rhs <- c("hdi_un","time","female_lfp_wb","ln_gni_wb","ln_maternal_mortality_wb",
         ...
         "life_expect_wb")
sapply(art_dat2[,..rhs], function(x) sum(is.na(x)))
...
rhs <- c("hdi_un","time","female_lfp_wb","ln_gni_wb","ln_maternal_mortality_wb",
         ...
         "life_expect_wb")
sapply(art_dat2[,..rhs], function(x) sum(is.na(x)))
lhs <- c("EA_2T8_AG25T99_F","EA_2T8_AG25T99_M","women_parl_wb","male_lfp_wb","ln_art_utilisation")
art_dat2[,id := as.numeric(as.factor(country))]
